...
import { httpErrorMiddleware, runAsyncConcurrently, canvasReactManager, memeReactManager } from 'shared/helpers';
...
describe('GET user/:id/daily-suggestions', () => {
  ...
  it('should not show most starred post if made 6 days ago', async () => {
    let res: any;
    const bigStar = 10000;
    const userInfo = await getNewAuthorizedUser();
  ...
  it('should show if you starred any of the content', async () => {
    const userInfo = await getNewAuthorizedUser();
    const newCanvas = await getPhonyCanvas(userInfo.user.id);
    await newCanvas.save();
    ...
    // Star someone elses meme //
    await memeReactManager('add', meme.id, userInfo.user.id);

    // Give the meme enough stars to show in suggested //
    meme.stars = 100;
    await meme.save();
    ...
    // Should show that you starred some content //
    res.body.forEach((value: ContentCard) => {
      if (value.type === EContentType.Canvas && value.id === newCanvas.id) {
        console.log(value);
      ...
      if (value.type === EContentType.MemeWithHost && value.cid === newCanvas.id) {
        console.log(value);
        expect(value.starred).toEqual(true);
      }
    });
